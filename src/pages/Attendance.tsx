...
// Generate mock attendance for the current week
...
export default function Attendance() {
  const [records] = useState<AttendanceRecord[]>(generateMockAttendance);
  ...
  const dayRecords = useMemo(() =>
    records
      .filter(r => r.date === selectedDate && r.workerName.toLowerCase().includes(search.toLowerCase()))
      .sort((a, b) => a.workerName.localeCompare(b.workerName)),
    [records, selectedDate, search]
  );

  // Overtime summary across all dates
  const overtimeSummary = useMemo(() => {
    ...
    records.forEach(r => {
      if (!summary[r.workerId]) {
        summary[r.workerId] = { name: r.workerName, role: r.role, department: r.department, totalHours: 0, overtimeHours: 0, daysWorked: 0 };
      }
      const hours = calcHours(r.checkIn, r.checkOut);
      if (hours > 0) {
        summary[r.workerId].totalHours += hours;
        summary[r.workerId].overtimeHours += calcOvertime(hours);
        summary[r.workerId].daysWorked += 1;
      }
    });
  ...
  const downloadCSV = () => {
    ...
    const rows = overtimeSummary.map(s =>
      `${s.workerId},${s.name},${s.role},${s.department},${s.daysWorked},${s.totalHours.toFixed(1)},${(s.daysWorked * 8).toFixed(1)},${s.overtimeHours.toFixed(1)}`
    ).join('\n');
    const blob = new Blob([header + rows], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `overtime-sheet-${dates[0]}-to-${dates[dates.length - 1]}.csv`;
    a.click();
          ...
          {dates.map(d => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium whitespace-nowrap transition-colors ${
                selectedDate === d ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'
              }`}
            >
      ...
      <div className="kpi-card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {dayRecords.map((r) => {
              ...
              return (
                <tr key={r.workerId} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="py-3">
                    {ot > 0 ? (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-warning/10 text-warning text-xs font-semibold">
                        <Clock className="h-3 w-3" />+{ot.toFixed(1)}h
                      </span>
                    ) : (
            ...
            {dayRecords.length === 0 && (
              <tr><td colSpan={6} className="py-8 text-center text-muted-foreground">No records for this date</td></tr>
            )}
          </tbody>
        </table>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-muted-foreground">
                ...
                <th className="pb-3 font-medium">Total Hrs</th>
                <th className="pb-3 font-medium">Standard Hrs</th>
                <th className="pb-3 font-medium">OT Hrs</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {overtimeSummary
                ...
                .map((s) => (
                  <tr key={s.workerId} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-2.5">
                      {s.overtimeHours > 0 ? (
                        <span className="text-warning font-semibold">+{s.overtimeHours.toFixed(1)}</span>
                      ) : (
