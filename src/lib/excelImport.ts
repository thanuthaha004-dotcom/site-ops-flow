...
export async function parseTripRequestsExcel(file: File): Promise<TripRequestRow[]> {
  ...
  return rows
    .map((r, i): TripRequestRow => ({
      ...
      expected_completion_time: normalizeTime(pick(r, 'Expected Completion Time', 'Expected Completion', 'expected_completion_time', 'ETA')),
      is_urgent: ['yes', 'y', 'true', '1', 'urgent'].includes(pick(r, 'Urgent', 'Urgent Requirement', 'is_urgent', 'urgent').toLowerCase()),
    }))
...
export function downloadTripRequestsTemplate() {
  ...
  const example = [
    {
      ...
      'Purpose of Trip': 'Pipe installation phase 2',
    ...
    },
    {
      ...
      'Vehicle Number': '',
      'Driver Name': '',
      'Purpose of Trip': 'Detection system testing',
      'Notes': '',
    },
  ];
  ...
  const instructions = [
    ...
    ['Trip No', 'No', 'Execution order for the day; lower runs first. Defaults to row order.'],
    ...
    ['Purpose of Trip', 'Yes*', 'Reason for the trip (e.g. installation, inspection, material pickup). Required when no passengers listed.'],
    ['Notes', 'No', 'Any additional remarks for the dispatcher.'],
  ];
